...
interface CalendarProps {
    ...
    view?: "month" | "week" | "day";
    className?: string;
    onNavigationHandlersReady?: (handlers: { onPrev: () => void; onNext: () => void; onToday: () => void; setView: (view: "month" | "week" | "day") => void }) => void;
    onViewChange?: (view: "month" | "week" | "day") => void;
}

export const Calendar = ({ events, view: externalView = "month", className, onNavigationHandlersReady, onViewChange }: CalendarProps) => {
    ...
    const [view, setView] = useState<string>(externalView);
    const [currentTime, setCurrentTime] = useState(() => now(timeZone));

    // Sync internal view state with external view prop
    useEffect(() => {
    ...
    const handleNavigate = useCallback(
        ...
        [view, selectedDate, currentMonthDate, locale, timeZone],
    );
    ...
    const handleViewChange = useCallback(
        (newView: string) => {
            setView(newView);
            if (onViewChange) {
                onViewChange(newView as "month" | "week" | "day");
            }
        },
        [onViewChange],
    );

    // Expose navigation and view handlers to parent component
    const navigationHandlers = useMemo(
        () => ({
            onPrev: () => handleNavigate("PREV"),
            onNext: () => handleNavigate("NEXT"),
            onToday: () => handleNavigate("TODAY"),
            setView: (newView: "month" | "week" | "day") => handleViewChange(newView),
        }),
        [handleNavigate, handleViewChange],
    );

    useEffect(() => {
        if (onNavigationHandlersReady) {
            onNavigationHandlersReady(navigationHandlers);
        }
    }, [onNavigationHandlersReady, navigationHandlers]);
