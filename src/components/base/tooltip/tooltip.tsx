...
import { Button as AriaButton, OverlayArrow as AriaOverlayArrow, Pressable, Tooltip as AriaTooltip, TooltipTrigger as AriaTooltipTrigger } from "react-aria-components";
...
interface TooltipTriggerProps extends Omit<AriaButtonProps, "elementType"> {
    /**
     * The element type to render. Use "div" or "span" when the tooltip trigger is inside another button to avoid nested button elements.
     * @default "button"
     */
    elementType?: "button" | "div" | "span";
}

export const TooltipTrigger = ({ children, className, elementType = "button", ...buttonProps }: TooltipTriggerProps) => {
    const baseClassName = (values: any) => cx("h-max w-max outline-hidden", typeof className === "function" ? className(values) : className);

    if (elementType === "div" || elementType === "span") {
        // Render as a div/span when inside another button to avoid nested buttons
        // The tooltip trigger doesn't need to be interactive - the parent button handles clicks
    ...
    return (
        <AriaButton {...buttonProps} className={baseClassName}>
            {children}
        </AriaButton>
    );
};
